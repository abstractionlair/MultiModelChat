...
One of the themes running through my repos, the posts I mean to put on my blog, and lots of conversations with different models (from which I'll mine much of the blog material) is about getting better results from AI by using multiple models. My first attempts at this involved having multiple chat apps up and copying reesponses from one into antoher and asking for a second opinion, moved on to the very manual process of copying both user and agent messages between the apps as the conversations unfolded, every model seeing every message. And this was genuinely helpful. Different ideas were generated by different models, and models found flaws in each others responses that they didn't catch themselves. (Though, this also multiplied the number of tokens and hence the cost. So it isn't clear this improved utility per dollar.) This app automated this and then started accruing other features like controlling reasoning token budgets and enabling tools like web search / grounding model by model. I used this or its predecesors in earnest for a while and planned more. For instance code execution is specced out. But Claude Hub [Add Link] has taken the slot this had filled. Multi-model collaboration by running Claude Code, Codex, and the Gemini CLI (now the Antigravity CLI) allowed me to get more done. And the Claude Hub MCP tools and the website on the VPS that hosts it have a newer, more flexible mechanism for chats like this. Also, this calls the inference APIs and the costs add up whereas using the coding harneses and my subscriptions is much more cost effective.
