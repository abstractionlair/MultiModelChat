...
One of the themes running through my repos, the posts I mean to put on my blog, and lots of conversations with different models (from which I'll mine much of the blog material) is about getting better results from AI by using multiple models. My first attempts at this involved having multiple chat apps open and copying responses from one into another and asking for second opinions. I moved on to the very manual process of copying both user and agent messages between the apps as conversations unfolded, every model seeing every message. And this was genuinely helpful. Different ideas were generated by different models, and models found flaws in each other's responses that they didn't catch themselves. (Though, this also multiplied the number of tokens and hence the cost. So it isn't clear this improved utility per dollar.) This app automated this and then started accruing other features like controlling reasoning token budgets and enabling tools like web search / grounding model by model. I used this or its predecessors in earnest for a while and planned more. For instance code execution was specced out. But [Claude Hub](https://github.com/abstractionlair/claude-hub) has taken the slot this had filled. Multi-model collaboration by running Claude Code, Codex, and the Gemini CLI (now the Antigravity CLI) allowed me to get more done. And the Claude Hub MCP tools and the website on the VPS that hosts it have a newer, more flexible mechanism for chats like this. Also, this calls the inference APIs and the costs add up whereas using the coding harnesses and my subscriptions is much more cost effective.

## Introduction

The description from here on was written mostly by Claude based on reading the files and the docs.

This app enables having one conversation with several frontier models at once: each turn fans out to the models you've configured — Claude, GPT, Gemini, Grok — and each model sees the full history, including what the other models said. Conversations belong to projects, files can be attached for context, and both files and conversation history are full-text searchable. There's a live sandbox at https://chat.abstractionlair.xyz.

The next phase — code execution, so models can work on attached files rather than just read them — was specced but not started. The public sandbox deliberately runs cheap models (GPT-5 nano, Gemini 2.5 Flash-Lite) behind cost controls; run your own instance with your own keys for the full lineup.
